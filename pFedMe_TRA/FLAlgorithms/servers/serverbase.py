...
class Server:
    ...
    def add_parameters_with_droprate(self, user, ratio,drop_rate):
        ...
        for server_param, user_param in zip(self.model.parameters(), user.get_parameters()):
            ...
            server_param.data = server_param.data + user_param.data.clone() * bernoulli_mask * ratio / (1-drop_rate)
    ...
    def personalized_aggregate_parameters_with_droprate(self,drop_rate):
        assert (self.users is not None and len(self.users) > 0)
